...
class AudioService:
    ...
    Args:
        control (str): Control de audio a utilizar (por defecto "Master").
    Returns:
        int: Volumen actual (0-100) o -1 en caso de error.
    Raises:
    ...
    def is_muted(self, control: str = DEFAULT_CONTROL) -> bool:
        """Devuelve True si el canal de audio está muteado. 
        Args:
            control (str): Nombre del control de audio, por defecto es DEFAULT_CONTROL.
        Returns:
    ...
    def get_controls(self) -> list[str]:
        """Devuelve una lista de controles amixer disponibles. 
        Args: 
            No aplica, método sin parámetros.
        Returns: 
            Una lista de strings con los controles disponibles.
        Raises: 
            Excepciones generales en caso de error, retornando un control predeterminado."""
        try:
            out = subprocess.check_output(
                ["amixer", "scontrols"],
                stderr=subprocess.DEVNULL,
