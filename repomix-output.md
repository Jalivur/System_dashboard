...
- Only files matching these patterns are included: ui/windows/*.py, ui/styles.py, ui/widgets.py, config/settings.py, config/button_labels.py
...
## File: ui/windows/hardware_info_window.py
```python
"""
...
## File: ui/windows/crontab_window.py
```python
"""
...
## File: ui/windows/button_manager_window.py
```python
"""
...
class HomebridgeWindow(ctk.CTkToplevel):
    ...
    def _render(self, accessories):
        ...
        if self.hb.is_reachable():
            ...
            self._set_status(
                f"{total} dispositivo{'s' if total != 1 else ''} "
    ...
    def _card_switch(self, card, acc, disabled):
        ...
        def on_toggle(new_state, uid=acc["uniqueId"]):
            if read_only:
                custom_msgbox(
                    self,
                    f"El dispositivo '{acc['displayName']}' no puede ser manipulado.",
                    "Aviso",
                )
                return
    ...
    def _card_thermostat(self, card, acc, disabled):
        ...
        target_var = ctk.StringVar(master=self, value=f"{acc['target_temp']:.1f}°C")
    ...
    def _card_blind(self, card, acc, disabled):
        ...
        pos_var = ctk.IntVar(master=self, value=acc["position"])
